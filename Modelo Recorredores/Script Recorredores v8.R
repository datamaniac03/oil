...
setwd("C:/Users/mferrari/Desktop/R - Demo - Recorredores")

datos = read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 1, header = FALSE)
variables = read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 2, header = FALSE)
rutas = read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 3, header = TRUE)
puntos = read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 4, header = TRUE)
coord = read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 6, header = TRUE)

# read table with categories display
categorias <- read.xls ("Modelo de Recorredores vFinal.xlsm", sheet = 8, header = TRUE)
...
puntos <- puntos[puntos$Activo=='Si',c(3,6,7)]
...
#remove category fuera from puntos
...
while (k <= iterations) {
  ...
  #### change this cond
  if (categorias[as.character(puntos[node,'Categoria']),'visitas']>1) {
...
while (t < 3*iterations) {
  ...
  cat_more1 <- as.character(categorias$cat[categorias$visitas>1])
  go.on <- !(puntos[node,"Categoria"] %in% cat_more1)
...
#correct base n.points <- 0
data.out[data.out$pozos.trab=='Base','n.points'] <- 0
...
wb <- loadWorkbook("Modelo de Recorredores vFinal.xlsm")
sheets <- getSheets(wb)
...
saveWorkbook(wb, "Modelo de Recorredores vFinal.xlsm")
