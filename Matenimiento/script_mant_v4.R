...
#replace 42-S by 42S
baterias <- gsub("-","",unique(puntos$Bateria))
...
#### 00. incorporate specific place and transport time ####
...
#posoz and baterias are assigned randomply
...
#### 04. initialize the calendar of mantenience task corrective ####
...
#### 05. create a summary of tasks to complete ####
...
#### 06. function create daily record ####
...
#### 07. call the function to generate the record ####
num_cuad_loop <-seq(5,40,5)
out <- NULL
...
for (num_cuad in num_cuad_loop) {
  ...
  names(corr_times) <- c('dpy_corr_alta_agua','dpy_corr_alta_bat','dpy_corr_alta_gas','dpy_corr_alta_pet',
                         'dpy_corr_alta_pozo',
                         'dpy_corr_med_agua','dpy_corr_med_bat','dpy_corr_med_gas','dpy_corr_med_pet',
                         'dpy_corr_med_pozo')
  
  line <- c(line,corr_times)
...
rownames(out)<-NULL
WriteXLS(as.data.frame(out), "data_out.xlsx")
