...
day_record <- function(day,num_cuad) {
      ...
      if (length(cuad_id)>0) {
        ...
        if(!is.na(task$task_id)) {
          ...
          if(!is.na(task$task_id)) {
            ...
            task <- get_open_task (stack_tot, day + 17/24, TRUE)
            #print('corrective task:')
            #print(task)
  ...
  #print(cuad_record)
  #print(stack_tot[1:10,])
  }
}

#### 05. initialize list of pozos, baterias, satelites, plantas ####
...
for (tar_id in 2){ ## principal loop tar_id <- 2
#tar_id <- 2
...
#### 07. intitialize list of nodes: pozos, baterias and plantas ####
...
#### 08. initialize the calendar of manteniance task predictive and preventive ####
...
#### 09. incorporate specific place and transport time ####
...
#### 10. initialize the calendar of mantenience task corrective ####
...
#### 11. incorporate specific place and transport time ####
#calculate trans time
...
#### 12. create a summary of tasks to complete ####
...
total_ava_time <- num_cuad*years2eval*length(labor_days_id)*8
total_ava_time_py <- num_cuad*length(labor_days_id)*8

#task not completed
task_not_completed_time <- sum(stack_tot$total_time[!stack_tot$completed])
...
directory <- '/Users/xavierign/Documents/oil/Modelo Matenimiento/out_asignacion/'
