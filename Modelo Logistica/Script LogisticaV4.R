...
colnames(tareas_log) <- c('grupo','actividad','prioridad','tarea','turno','weekdays',
                          'fluido','unidad','demanda','tiempo_car-des',
                          'comienzo','fin')
...
assign <- function (id_task,id_vehicle,ini_eq_free) {
  ...
  #correct capacities
  if (vehicle$tipo=='Articulado Semi'&(task$grupo=='Perforacion'|(length(grep("edit cap",task$tarea))>0))) {
    capacity <- 35
  }

  if (vehicle$tipo=='Camion con Hidro'&task$grupo=='Tubing') {
    capacity <- 10
  }
  
  if (vehicle$tipo=='Camion con Hidro'&task$tarea=='Recupero') {
    capacity <- 8
  }
  #update stack_log
  if(capacity > task$demanda_open) {
    #complete task
    ...
    #incomplete task
    stack_log[id_task,'demanda_open']<<-  task$demanda_open-capacity
  }
...
#### 06.5 add_rest_vehicle(type_uni) function ####

add_rest_vehicle <- function(type_uni) {
  #add descanzo to stack_log
  if(ncol(stack_log)==11) {
    ...
    rows_descanzo <-data.frame('Descanzo',1,1,'descanzo',type_uni,0.1,1:day_iteration,
                               NA,(1-recursos[type_uni,'Disponibilidad'])*24,0.1,NA,NA)
  }
  colnames(rows_descanzo) <- colnames(stack_log)
  stack_log <<- rbind(stack_log,rows_descanzo)
}

reset_stack_log <- function(type_uni) {
  same_type_tasks_ids <- grepl(type_uni, stack_log$unidad)
  stack_log[same_type_tasks_ids,'comp_dia'] <<- NA
  #stack_log[same_type_tasks_ids,'dif'] <<- NA
}
...
print('')
print(paste(c('assigning the vehicle type: ', type_uni ),collapse=''))
...
row <- 1
  ...
  for (num_vehiculo in 1:total_vehic){
    ...
    recursos_record[row,] =  c(tipo=type_uni, id=row,capacity= recursos[type_uni,'Capacidad'],free=1)
    row <- row +1
  }
...
if (perc_not_comp < 0.01) {
  ...
  print(paste(c('vehicle: ', type_uni, ', minimum quantity to complete the tasks: ',total_vehic ), collapse = ""))

  if(type_uni=='Potable') {
    print(paste(c('adjusting quantities to an 8 hours day: ', floor(total_vehic/3)+1),collapse=""))
  }
