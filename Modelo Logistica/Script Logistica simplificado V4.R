...
tareas_log <- tareas_log[,c(1:2,4:14,16:18)]
colnames(tareas_log) <- c('grupo','actividad','prioridad','tarea','rango_ini','rango_fin','tol','respuesta','turno','weekdays',
...
stack_log <- data.frame(act=character(), prio = numeric(), act_id=numeric(), 
                        tarea=character(),rango_ini= numeric(), 
                        rango_fin = numeric(),
                        tol= character(), respuesta= numeric(),
                        unidad=character(), demanda= numeric(), 
...
add_task_stack <- function (type_uni,day, type_number) {
  ...
  #if it is random correct the day
  mat$day[mat$tol=='Random'] <- mat$day[mat$tol=='Random'] + runif(sum(mat$tol=='Random'))
  ...
  stack_log <<- rbind(stack_log, 
                      mat[,c( 'grupo','prioridad','type_number','tarea','rango_ini', 'rango_fin', 'respuesta',
                              'unidad', 'demanda', 'day', 'loc','tiempo_total', 'demanda_open','day_open','comp_dia')])
}
...
for (cat_pozo in frecuencia_purgas$Categoria) {
  ...
  for (day_iteration in seq(1,total_days,frecuencia_purgas[cat_pozo,'dias'])) {
    rows2add <- cbind.data.frame(grupo=paste(c('Purga',cat_pozo),collapse='_'),
                                 ...
                                 type_number=(1:nrow(t.m)), tarea='visita purga', 
                                 rango_ini=0,
                                 rango_fin=24,
                                 respuesta=9999,
...
recursos_record$tipo <- rep(recursos$Tipo.Unidad,recursos$Cantidad)
recursos_record$id <- sequence(recursos$Cantidad)
recursos_record$capacity <- rep(recursos$Capacidad,recursos$Cantidad)
recursos_record$free <- 1

#add rest ###
add_rest_vehicle <- function(type_uni) {
    rows_descanzo <-data.frame('Descanzo',1,1,'descanzo',0,24,24,type_uni,0.1,1:day_iteration,
                               NA,(1-recursos[type_uni,'Disponibilidad'])*24,0.1,1:day_iteration,NA)
  colnames(rows_descanzo) <- colnames(stack_log)
  stack_log <<- rbind(stack_log,rows_descanzo)

}

for (transport in recursos$Tipo.Unidad) {
  print(transport)
  add_rest_vehicle(transport)
}

### save the data
save(stack_log, recursos_record, file = "logData.RData")
