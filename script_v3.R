...
while (t < 1000) {
  ...
  id2extract <- sample(1:nrow(t.m),1, prob =(t.m[,'n.points']-1))
  
  #select a node to extract
  route2exctract <- sol[[id2extract]]
  node2extract.id <- sample(1:length(route2exctract),1,prob=tiempos.saved(route2exctract))
  node2extract <- route2exctract[node2extract.id]
  
  #select a tour to add
  id2add <- sample(1:nrow(t.m),1, prob =abs(7-t.m[,'n.points']))
